...
# אירועים - הוספת עמודת תווית ממוספרת מראש כדי למנוע שגיאות בגרפים
events <- tribble(
  ~date, ~label,
  ...
  mutate(
    date = ymd(date),
    display_label = paste(row_number(), label, sep=". ") # יצירת הטקסט לתצוגה מראש
  )

# ======================================================================
# 1. קריאת נתונים, זיהוי וסינון
# ======================================================================
# קריאת הקובץ
df <- read_csv(FILE_PATH, locale = locale(encoding = "UTF-8"))

# פונקציית עזר להמרת טקסט לסנטימנט מספרי
...
df_weekly <- df_clean %>%
  ...
  # חישוב הסנטימנט הממוצע המשוקלל רק אם יש מספיק כתבות
...
df_ts <- df_weekly %>%
  ...
  arrange(week) %>%
  mutate(across(-week, ~ na.approx(., na.rm = FALSE, maxgap = 3))) %>%
  mutate(across(-week, ~ na.locf(., na.rm = FALSE))) %>% 
  mutate(across(-week, ~ na.locf(., fromLast = TRUE, na.rm = FALSE))) %>% 
  mutate(across(-week, ~ rollmean(., k = SMOOTH_WEEKS, fill = NA, align = "right"))) %>%
  pivot_longer(cols = -week, names_to = "source_name", values_to = "smooth_sentiment") %>%
  drop_na(smooth_sentiment)

# ======================================================================
# 3. יצירת הגרפים ושמירתם
# ======================================================================

# גרף 1: סקירה כללית - כל האתרים יחד
plot_overview <- ggplot(df_ts, aes(x = week, y = smooth_sentiment, color = source_name)) +
  ...
  # שימוש בעמודת display_label שהכנו מראש
...
# גרף 2: פיצול לגרף נפרד עבור כל אתר (Facets)
...
# גרף 3: לפני מול אחרי ה-7 באוקטובר
...
# ======================================================================
# 4. בדיקת מובהקות סטטיסטית סביב אירועים
# ======================================================================
